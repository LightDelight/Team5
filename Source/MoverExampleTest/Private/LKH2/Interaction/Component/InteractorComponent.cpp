...
void UInteractorComponent::GetLifetimeReplicatedProps(
  ...
  DOREPLIFETIME(UInteractorComponent, CarriedActor);
}

void UInteractorComponent::OnRep_CarriedActor(AActor *OldCarriedActor) {
...
void UInteractorComponent::SetTarget(AActor *NewTarget) {
  ...
  if (CurrentTarget != nullptr &&
      CurrentTarget->Implements<UInteractionContextInterface>()) {
    if (bIsLocalPlayer) {
      IInteractionContextInterface::Execute_SetOutlineEnabled(CurrentTarget, false);
    }
  }
  ...
  if (CurrentTarget != nullptr &&
      CurrentTarget->Implements<UInteractionContextInterface>()) {
    if (bIsLocalPlayer) {
      IInteractionContextInterface::Execute_SetOutlineEnabled(CurrentTarget, true);
    }
  }
}

void UInteractorComponent::TryPickupOrDrop() {
...
bool UInteractorComponent::Server_TryPickupOrDrop_Validate(AActor *Target) {
  return true;
}
void UInteractorComponent::Server_TryPickupOrDrop_Implementation(AActor *Target) {
  bWantsPickupOrDrop = true;
  InputBufferTimer = InputBufferTimeLimit;
  ProcessInputBuffer(Target); // 클라이언트가 알려준 타겟으로 실행
}

void UInteractorComponent::TryThrow() {
  bWantsThrow = true;
  InputBufferTimer = InputBufferTimeLimit;
  ProcessInputBuffer(CurrentTarget);
...
bool UInteractorComponent::Server_TryThrow_Validate(AActor *Target) { return true; }
void UInteractorComponent::Server_TryThrow_Implementation(AActor *Target) {
  bWantsThrow = true;
  InputBufferTimer = InputBufferTimeLimit;
  ProcessInputBuffer(Target);
}

AActor *UInteractorComponent::GetCarriedActor() const { return CarriedActor; }

void UInteractorComponent::ForceDrop() {
  if (CarriedActor) {
...
void UInteractorComponent::ForceEquip(AActor *ItemToEquip) {
...
FInteractionContext UInteractorComponent::CreateInteractionContext(AActor *Target,
  ...
  FInteractionContext Context(Cast<AActor>(GetOwner()), Type);
  ...
  if (Type == EInteractionType::Throw) {
...
void UInteractorComponent::ProcessInputBuffer(AActor *Target) {
  if (bWantsPickupOrDrop) {
    if (CarriedActor) {
      if (Target && Target->Implements<UInteractionContextInterface>()) {
        IInteractionContextInterface::Execute_OnInteract(
            Target, CreateInteractionContext(Target, EInteractionType::Interact));
      } else {
        IInteractionContextInterface::Execute_OnInteract(
            CarriedActor, CreateInteractionContext(nullptr, EInteractionType::Interact));
        AActor *OldItem = CarriedActor;
        CarriedActor = nullptr;
        OnRep_CarriedActor(OldItem);
      }
      ...
      if (Target && Target->Implements<UInteractionContextInterface>()) {
        FInteractionContext Context = CreateInteractionContext(Target, EInteractionType::Interact);
        bool bSuccess = IInteractionContextInterface::Execute_OnInteract(Target, Context);
  ...
  if (bWantsThrow && CarriedActor) {
    if (CarriedActor->Implements<UInteractionContextInterface>()) {
      // 던지기 처리 (로직 모듈에 위임)
      IInteractionContextInterface::Execute_OnInteract(
          CarriedActor, CreateInteractionContext(nullptr, EInteractionType::Throw));
    }
    AActor *OldItem = CarriedActor;
    CarriedActor = nullptr;
    OnRep_CarriedActor(OldItem);
