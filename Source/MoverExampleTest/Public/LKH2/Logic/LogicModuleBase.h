...
public:
  ...
  virtual bool OnLogicInteract_Implementation(const FCarryContext &Context) override;

  /**
   * 상호작용 실행을 위한 템플릿 메서드.
   * 사전 검사 -> 핵심 실행 -> 사후 처리 과정을 캡슐화합니다.
   */
  virtual bool ExecuteInteraction(const FCarryContext &Context);
...
protected:
   ...
   */
  virtual bool PreInteractCheck(const FCarryContext &Context) { return true; }

  /**
   * [Core-Logic] 실제 상호작용 행동을 수행합니다.
   */
  virtual bool PerformInteraction(const FCarryContext &Context) { return false; }

  /**
   * [Post-Logic] 상호작용 실행 후(성공/실패 무관) 정리 및 부가 작업을 수행합니다.
   */
  virtual void PostInteract(const FCarryContext &Context, bool bExecutionResult) {}
