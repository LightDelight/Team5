...
* **구조 (Template Method Pattern)**: 로직의 실행 흐름은 `ExecuteInteraction`이라는 표준 템플릿에 의해 강제됩니다. 개별 모듈은 가상 함수 훅(`PreCheck`, `Perform`, `Post`)만을 구현하여 비즈니스 로직의 파편화를 방지합니다.
...
* **UCarryComponent**: 플레이어(Interactor)에 부착되어 들기/내려놓기/던지기 입력 버퍼링을 처리하고, 캐스트 대신 인터페이스를 통해 대상에게 메시지를 전달(Dispatch)하는 역할을 합니다.  
* **UCarryableComponent**: 대상 아이템에 부착되어 해당 아이템이 들릴 수 있는 객체임을 나타내며, **데이터-애그노스틱(Data-Agnostic) 실행 엔진**으로서 소유자(Owner)가 주입해준 로직 모듈들을 순회하며 실행합니다.
* **UCarryInteractComponent**: 워크스테이션(작업대 등)에 부착되어 상호작용 메시지를 수신합니다. 상속이나 데이터 에셋에 의존하지 않고, 런타임에 전달받은 모듈 리스트를 실행하는 범용 인터페이스 역할을 수행합니다. 자체적으로 **FLogicBlackboard**를 호스팅합니다.
...
* **원칙**: 모든 상호작용은 `ULogicModuleBase::ExecuteInteraction`이 강제하는 **사전 로직(Pre-Check) → 핵심 실행(Perform) → 사후 로직(Post-Interact)**의 3단계 템플릿 메서드 흐름을 따릅니다.
* **사전 로직 (Pre-Logic)**:  
  * 행동을 실행하기 **전에** 수행하는 검증·준비 단계입니다. (`PreInteractCheck` 가상 함수)
  * "플레이어 손이 비어있는가?", "워크스테이션에 빈 슬롯이 있는가?", "데이터적으로 허용된 액션인가(bAllowStore 등)?" 등의 **Validation(유효성 검증)**을 수행합니다.
  * 사전 로직에서 조건이 불충분하면 핵심 실행으로 진입하지 않고 **즉시 false를 반환**하여 부작용(Side Effect)을 원천 차단합니다.  
* **핵심 실행 (Core Execution)**:  
  * 사전 로직을 모두 통과한 경우에만 도달하는 **실제 행동** 단계입니다. (`PerformInteraction` 가상 함수)
  * 아이템 Attach, 상태 전이, 블랙보드 갱신 등 **되돌리기 어려운 부작용**이 여기서 발생합니다. (Manager 호출 등을 통해 이루어짐)  
* **사후 로직 (Post-Logic)**:  
  * 핵심 실행 이후 수행하는 정리·후처리 단계입니다. (`PostInteract` 가상 함수)
...
* **중간 검증(Guard)과 체이닝**:
  * 개별 모듈이 복잡한 분기문을 가지는 대신, 여러 개의 작은 모듈이 `Chain of Responsibility` 패턴으로 연결되어 각자의 영역만 검증합니다. 예: `Logic_CarryInteract_Common`이 거치 가능 여부만 체크하고 처리하면, 뒤따르는 `Combine` 로직은 실행되지 않습니다.

### **6. 컴포넌트와 데이터의 완전한 디커플링 (Data-Agnostic Isolation)**

* **원칙**: 상호작용 컴포넌트(`UCarryInteractComponent`, `UCarryableComponent`)는 본인이 어떤 데이터 에셋(`UWorkstationData`, `UItemData`)에 의해 정의되었는지 몰라야 합니다.
* **이유**: 컴포넌트가 특정 데이터 클래스에 의존하면, 데이터 구조가 바뀔 때마다 컴포넌트 코드를 수정해야 하며, 데이터가 없는 환경에서는 컴포넌트를 테스트할 수 없게 됩니다.
* **구현 방식**:
  1. **모듈 주입(Injection)**: `AWorkStationBase`나 `AItemBase` 같은 소유자가 `BeginPlay` 시점에 데이터 에셋에서 모듈 목록을 추출하여 컴포넌트의 `SetLogicModules()`를 호출합니다.
  2. **범용 인터페이스**: `ILogicContextInterface::GetLogicModules()`를 통해 어떤 객체로부터든 실행 가능한 로직 리스트를 추상화된 형태로 가져올 수 있습니다.
  3. **실행 대행**: 컴포넌트는 단지 전달받은 모듈 리스트를 "누가 보냈는지 묻지 않고" 순차적으로 실행하는 **순수 매커니즘**으로만 동작합니다.

### **7. 예외 없는 환경 조성 (Exception-Free Environment)**
...
### **8. 로직에서는 오직 판단과 호출만 (Logic = Judgment + Dispatch Only)**
...
### **9. 예외를 만들 경우 기술 부채 격리 (Technical Debt Isolation for Exceptions)**
